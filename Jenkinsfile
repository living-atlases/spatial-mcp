...
pipeline {
    ...
    parameters {
        ...
        booleanParam(name: 'KEEP_DEMO_LAYER', defaultValue: false, description: 'Also add the mcp_demo_* layers (synthetic regions, Australian states, Spanish autonomous communities) and leave them on the stack (for demos and screenshots; skipped if it exists; a redeploy removes it)')
    }
    stages {
        stage('Unit tests') {
            steps {
